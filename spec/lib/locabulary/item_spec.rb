...
RSpec.describe Locabulary::Item do
  ...
  context '.class_to_instantiate' do
    it "finds a defined subclass" do
      expect(described_class.class_to_instantiate(predicate_name: 'administrative_units')).to(
        eq(Locabulary::Items::AdministrativeUnit)
      )
    end

    it "defaults to the Items::Base" do
      expect(described_class.class_to_instantiate(predicate_name: 'chicken')).to(eq(Locabulary::Items::Base))
    end
  end
end
